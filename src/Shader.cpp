...
Shader::Shader(const std::string vertname, const std::string fragname, const std::string geoname)
{
    ...
    // load context of geometric shader
    file.open(("./Shaders/" + geoname).c_str());
    if(!file.is_open())
    {
        printf("Cannot load shader file source: %s\n", geoname.c_str());
        this->programID = 0;
    ...
    sstr << file.rdbuf();
    std::string geosourceS = sstr.str();
    const char *geosource = geosourceS.c_str();
    file.close();
    file.clear();
    sstr.str("");
