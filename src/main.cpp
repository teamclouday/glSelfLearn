...
void renderAll(float deltaT, float fps)
{
    ...
    myData.C = glm::vec2((sinf(tt*0.1f)+cosf(tt*0.23f))*0.5f,
                         (cosf(tt*0.13f)+sinf(tt*0.21f))*0.5f);
    myShader->use();
    glBindTexture(GL_TEXTURE_1D, paletteTex);
    glBindVertexArray(VAO);
    glUniform2fv(glGetUniformLocation(myShader->programID, "offset"), 1, &myData.offset[0]);
    glUniform2fv(glGetUniformLocation(myShader->programID, "C"), 1, &myData.C[0]);
    glUniform1f(glGetUniformLocation(myShader->programID, "zoom"), myData.zoom);
...
int main(int argc, char *argv[])
{
    ...
    myShader = new Shader();
    myShader->add("./shaders/julia.vert", GL_VERTEX_SHADER);
    myShader->add("./shaders/julia.frag", GL_FRAGMENT_SHADER);
